...
| Per-user authorization | `WINBRIDGE_PRINCIPALS` (JSON array) | Each principal has its own token, role, and optional command policy. A key may be a plaintext `token`/`tokenEnv` or a SHA-256 `tokenHash` (WinBridge hashes the presented token and compares, so an external key store never needs to hold the plaintext). The legacy `WINBRIDGE_TOKEN` remains a single full-access admin. |

...
| Git Bash tool family | `WINREACH_ALLOW_BASH` (+ optional `WINREACH_BASH_PATH`) | The `bash_*` tools run commands through Git Bash's `bash.exe` instead of PowerShell. Bash is **an alternate execution surface under the exact same controls — not a policy bypass**: every `bash_execute`/`bash_send` command is evaluated against the global and per-principal allow/deny policy *before* bash is spawned, gated by the principal's `tools` allowlist, and written to the audit log with the right tool name. It is disabled by default and only registered when enabled and `bash.exe` resolves (auto-detected at the common Git-for-Windows paths, or `WINREACH_BASH_PATH`). Since the command policy regexes are matched against the raw command string, operators writing a denylist should account for POSIX syntax (e.g. `rm -rf`) as well as PowerShell cmdlets. |

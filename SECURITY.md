...
| Rate limiting + per-principal quotas | `WINREACH_RATE_LIMIT_PER_MIN` / `WINREACH_RATE_LIMIT_DAILY_QUOTA` (+ per-principal/role `rateLimitPerMin` / `dailyQuota`) | A per-principal cap on **every** tool call: a per-minute token bucket and an optional daily quota (fixed window, resets at 00:00 UTC). Both are opt-in — `0`/unset disables them, so default deployments are unchanged. Global defaults apply to every principal and are overridable per principal or role (an explicit `0` opts a principal out). A call over budget never reaches the tool (or its command policy): it returns `{ "blocked": true, "reason": "rate limited" \| "quota exceeded", "retryAfter": <seconds> }` and is audited with `decision: "blocked"`. Limits are in-memory and per-process, keyed by principal (one principal's throttling never affects another); front WinReach with a proxy to bound usage across multiple instances. The finer-grained `computer_use` actions/sec cap remains as an inner limit. |

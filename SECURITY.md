...
- If you publish WinReach through a Cloudflare tunnel, prefer a **named tunnel** you own (`WINREACH_TUNNEL_TOKEN` + `WINREACH_TUNNEL_HOSTNAME`) so the endpoint is a stable hostname you control. WinReach never provisions or hosts a tunnel — you bring your own Cloudflare account, domain, and created tunnel. Treat `WINREACH_TUNNEL_TOKEN` as a secret (it encodes your account + tunnel id + secret); WinReach never logs it.
...
| Named (operator-owned) Cloudflare tunnel | `WINREACH_TUNNEL_TOKEN` + `WINREACH_TUNNEL_HOSTNAME` | Publishes WinReach on a **stable, operator-owned hostname** instead of a random `*.trycloudflare.com` that changes every restart. **WinReach never provisions, hosts, or owns any tunnel** — the operator brings their own Cloudflare account, domain/zone, created named tunnel, and DNS route, and WinReach only runs `cloudflared tunnel run --token <token>`. Both env vars are required together (one without the other fails fast at startup). `WINREACH_TUNNEL_TOKEN` is a secret and is never written to logs. Tunnel traffic still terminates TLS at Cloudflare and forwards to WinReach over loopback — in-app TLS/mTLS does not apply to tunnel traffic; the bearer token remains the control over the tunnel. |

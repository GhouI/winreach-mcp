// Shared WINREACH_SETUP_KEY bearer authorization, matching app/api/config.
// The guarded endpoint stays disabled until the operator sets WINREACH_SETUP_KEY
...
export function authorizeSetupKey(req: NextRequest): NextResponse | null {
  ...
  if (!expected) {
    return NextResponse.json(
      {
        error:
          "Setup API disabled: set WINREACH_SETUP_KEY on the host running setup-web to enable it.",
      },
      { status: 503 },
    );
  }
